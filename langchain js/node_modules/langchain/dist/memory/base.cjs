...
function getBufferString(messages, human_prefix = "Human", ai_prefix = "AI") {
    ...
    for (const m of messages) {
        let role;
        if (m instanceof index_js_1.HumanChatMessage) {
            role = human_prefix;
        }
        else if (m instanceof index_js_1.AIChatMessage) {
            role = ai_prefix;
        }
        else if (m instanceof index_js_1.SystemChatMessage) {
            role = "System";
        }
        else if (m instanceof index_js_1.ChatMessage) {
            role = m.role;
        }
        else {
